...
public class RouteController {
    ...
    private void handleTestAction(RouteAction action, String route){
        ...
        try {
            switch (action) {
                case SET_ROUTE:
                    deps.getRouteService().setRouteById(route);
                    break;
                case NAVIGATE:
                    deps.getRouteService().navigationById(route);
                    break;
                case UNSET_ROUTE:
                    deps.getRouteService().unsetRouteById(route);
                    break;
                case SET_LOW_SPEED:
        ...
        new Thread(() -> {
            for (String o : selectedRoutesList) {
                ...
                try {
                    switch (action) {
                        case SET_ROUTE:
                            deps.getRouteService().setRouteById(o);
                            break;
                        case NAVIGATE:
                            deps.getRouteService().navigationById(o);
                            break;
                        case UNSET_ROUTE:
                            deps.getRouteService().unsetRouteById(o);
                            break;
                        case SET_LOW_SPEED:
    ...
    public void initialize()  {
        ...
        setRouteButton.setOnAction(event -> handleRouteAction(RouteAction.SET_ROUTE));
        unsetRouteButton.setOnAction(event -> handleRouteAction(RouteAction.UNSET_ROUTE));
