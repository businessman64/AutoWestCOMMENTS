...
//		});
		...
		berth.setOnAction(event -> {
			// Redirect to tracks page
			if (currentView != null) {
				FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/BerthView.fxml"));
				BerthController berthController = null;
				try {
					logger.info("---------------------------------berth---------------------------------");

					berthController = new BerthController(stage, deps);
				} catch (ParserConfigurationException e) {
					throw new RuntimeException(e);
				} catch (IOException e) {
					throw new RuntimeException(e);
				...
				loader.setController(berthController);
				HBox root = null;
				try {
					root = loader.load();
				} catch (IOException e) {
				...
				berthController.showStage(root);
				//stage.close();
			}else{
				getConfigurationScreen();
	...
	public void getConfigurationScreen(){
		...
		FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/ConfigureHomeView.fxml"));

		try {
			//ConfigureScreenController configureScreenController = new ConfigureScreenController(stage, deps);
			ConfigureHomeControl configureScreenController = new ConfigureHomeControl(stage, deps);
